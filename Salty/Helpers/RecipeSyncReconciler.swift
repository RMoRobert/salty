...
enum RecipeSyncReconciler {
    ...
    struct Plan: Equatable {
        var toUpload: [String] = []         // local is new or newer → push to server
        var toDownload: [String] = []       // server is new or newer → pull to local
        var toDeleteLocally: [String] = []  // existed before lastSync, now gone on server → delete here
        var toDeleteOnServer: [String] = [] // existed before lastSync, now gone locally → delete there
    }
    ...
    static func plan(local: [Entry], server: [Entry], isFirstSync: Bool, lastSyncDate: Date?) -> Plan {
        var plan = Plan()
