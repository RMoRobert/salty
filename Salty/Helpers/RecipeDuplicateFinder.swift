...
struct RecipeDuplicateGroup: Identifiable, Equatable, Sendable {
    ...
    var id: String { recipes.first?.id ?? "" }
    ...
    var name: String { recipes.first?.name ?? "" }
}

enum RecipeDuplicateFinder {
    ...
    static func matchKey(for recipe: Recipe, level: RecipeDuplicateMatchLevel) -> MatchKey {
        switch level {
        case .titleAndSource:
            return .fields(
                FieldKey(
    ...
    /// not compared.
    static func normalized(_ recipe: Recipe) -> Recipe {
