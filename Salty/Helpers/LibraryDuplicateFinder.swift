...
enum LibraryItemKind: String, CaseIterable, Identifiable, Sendable {
    case category
    case course
    case tag
...
struct LibraryDuplicateItem: Identifiable, Hashable, Sendable {
    let id: String
    let name: String
    let recipeCount: Int
}
...
struct LibraryDuplicateGroup: Identifiable, Hashable, Sendable {
    ...
    let survivor: LibraryDuplicateItem
    ...
    let duplicates: [LibraryDuplicateItem]

    var id: String { "\(kind.rawValue)_\(survivor.id)" }
    var name: String { survivor.name }

    /// How many rows this group removes.
    var removedCount: Int { duplicates.count }
}

enum LibraryDuplicateFinder {

    /// Which row of a same-named set is kept.
    enum SurvivorRule: Sendable {
