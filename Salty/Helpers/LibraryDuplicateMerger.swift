...
enum LibraryDuplicateMerger {
    ...
    struct Summary: Equatable, Sendable {
        ...
        var touchedRecipes = 0

        var isEmpty: Bool { mergedGroups == 0 }
    }
    ...
    @discardableResult
    static func consolidateDuplicates(
        in db: Database,
    ...
    static func items(kind: LibraryItemKind, in db: Database) throws -> [LibraryDuplicateItem] {
    ...
    @discardableResult
    static func merge(_ groups: [LibraryDuplicateGroup], in db: Database) throws -> Summary {
        var summary = Summary()
