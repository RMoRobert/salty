...
//        do {
//            let recipeTags = try database.read { db in
//                try RecipeTag
//                    .filter(RecipeTag.Columns.recipeId == recipe.id)
//                    .including(required: RecipeTag.tag)
//                    .fetchAll(db)
//            }
//            
//            tags = recipeTags.map { $0.tag.name }.sorted { $0.localizedCaseInsensitiveCompare($1) == .orderedAscending }
//        } catch {
//            tags = []
//        }
//    }
